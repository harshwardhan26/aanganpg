...
export default async function FeesPage({
  ...
  const rows = students.map((student) => {
    ...
    return {
      ...
      collected,
      status,
      remindersSent: statement?.remindersSent ?? 0,
    };
  });
    ...
    <div className="flex flex-col gap-6">
      <div className="flex items-center justify-between gap-3">
        <Link
          ...
          className="flex min-h-12 items-center rounded-xl border-2 border-border px-4 text-base font-medium text-text-muted hover:bg-muted"
        >
      ...
      <div className="flex flex-wrap gap-2">
        <Link
          href={`/mess/${messId}/fees/reminders?month=${monthQuery}`}
          className="inline-flex min-h-11 items-center rounded-xl bg-primary-strong px-4 text-sm font-semibold text-white"
        >
          Send reminders
        </Link>
        <Link
          href={`/api/mess/${messId}/collections?month=${monthQuery}`}
          className="inline-flex min-h-11 items-center gap-2 rounded-xl border-2 border-border bg-white px-4 text-sm font-semibold text-text-main hover:bg-muted"
        >
          <Download className="h-4 w-4" aria-hidden /> Export CSV
        </Link>
        <Link
          href={`/mess/${messId}/more/reports?month=${monthQuery}`}
          className="inline-flex min-h-11 items-center rounded-xl border-2 border-border bg-white px-4 text-sm font-semibold text-text-main hover:bg-muted"
        >
          Print report
        </Link>
      </div>
      ...
      <form className="rounded-2xl border-2 border-border bg-white p-4" role="search">
        ...
        <label htmlFor="collection-search" className="text-sm font-semibold text-text-main">
          Find a student
        </label>
        <div className="mt-2 flex gap-2">
          <div className="relative min-w-0 flex-1">
            <Search
              className="pointer-events-none absolute top-3.5 left-3 h-5 w-5 text-text-muted"
              aria-hidden
            />
            <input
              ...
              name="q"
              defaultValue={search}
              className="min-h-12 w-full rounded-xl border-2 border-border bg-white pr-3 pl-10 text-base text-text-main"
              placeholder="Type a name"
            />
          </div>
          <button className="min-h-12 rounded-xl bg-primary-strong px-5 text-base font-semibold text-white">
            Search
          </button>
        </div>
        <div className="mt-3 flex gap-2 overflow-x-auto pb-1">
          <Filter
            href={`/mess/${messId}/fees?month=${monthQuery}`}
            active={!statusFilter}
            label="All"
          />
      ...
      {filtered.length === 0 ? (
        ...
        <ul className="flex flex-col gap-3">
          {filtered.map((row) => (
            <li key={row.id}>
              <Link
                href={`/mess/${messId}/fees/${row.id}?month=${monthQuery}`}
                className="flex min-h-24 items-center justify-between gap-3 rounded-2xl border-2 border-border bg-white p-4 transition-colors hover:bg-muted"
              >
                <div className="min-w-0">
                  <p className="truncate text-lg font-semibold text-text-main">
                    {row.name}
                    {row.leftAt && (
                      <span className="ml-2 text-sm font-normal text-text-muted">left</span>
                    )}
                  </p>
...
function Stat({ label, value, alert }: { label: string; value: string; alert?: boolean }) {
  return (
    <div className="rounded-2xl border-2 border-border bg-white p-3 sm:p-4">
      <p
        className={
          alert
            ? "font-heading text-xl font-bold tabular-nums text-red-800 sm:text-2xl"
            : "font-heading text-xl font-bold tabular-nums text-text-main sm:text-2xl"
        }
      >
        {value}
      </p>
...
function Filter({ href, active, label }: { href: string; active: boolean; label: string }) {
  return (
    <Link
      ...
      className={
        active
          ? "flex min-h-10 shrink-0 items-center rounded-full bg-text-main px-4 text-sm font-semibold text-white"
          : "flex min-h-10 shrink-0 items-center rounded-full bg-muted px-4 text-sm font-semibold text-text-muted"
      }
    >
      {label}
    </Link>
