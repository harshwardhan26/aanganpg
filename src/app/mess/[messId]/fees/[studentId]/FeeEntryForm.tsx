  ...
  return (
    <form action={action} className="rounded-2xl border-2 border-border bg-white p-5">
      ...
      <p className="mt-1 text-sm text-text-muted">
        Record money received or an owner-approved adjustment. Existing entries are never
        overwritten.
      </p>
      {state.message && (
        <p
          role="status"
          className="mt-3 rounded-xl bg-green-100 px-4 py-3 text-sm font-medium text-green-900"
        >
          {state.message}
        </p>
      )}
      ...
      <div className="mt-4 grid gap-4 sm:grid-cols-2">
        ...
        <label className="flex flex-col gap-1.5 text-sm font-semibold text-text-main">
          ...
          <input
            ...
            className="min-h-12 rounded-xl border-2 border-border px-3 text-base font-normal uppercase"
            placeholder="Required for UPI"
          />
        </label>
      </div>
      <label className="mt-4 flex flex-col gap-1.5 text-sm font-semibold text-text-main">
        Note
        <input
          name="note"
          className="min-h-12 rounded-xl border-2 border-border px-3 text-base font-normal"
...
export function ReminderForm({ messId, paymentId }: { messId: string; paymentId: string }) {
  ...
  return (
    <form action={action} className="rounded-2xl border-2 border-border bg-white p-5">
      ...
      <p className="mt-1 text-sm text-text-muted">
        Send the registered SMS template to the parent number.
      </p>
      {state.message && (
        <p role="status" className="mt-3 text-sm font-medium text-green-900">
          {state.message}
        </p>
      )}
      {!state.ok && (
      ...
      <button
        disabled={pending}
        className="mt-4 min-h-12 rounded-xl border-2 border-primary-strong px-5 text-base font-semibold text-primary-strong disabled:opacity-60"
      >
        {pending ? "Sending…" : "Send SMS reminder"}
      </button>
    </form>
  );
}
