...
export default async function StudentFeeLedger({
  params,
  searchParams,
}: {
  params: Promise<{ messId: string; studentId: string }>;
  ...
  const month =
    rawMonth && /^\d{4}-\d{2}$/.test(rawMonth)
      ? new Date(`${rawMonth}-01T00:00:00.000Z`)
  ...
  const status = feeStatus({
    charge,
    balance,
    due: dueDate(month, student.mess.dueDay),
    today: attendanceDay(new Date()),
    hasPayment:
      statement?.entries.some((entry) => entry.kind === "PAYMENT" && !entry.reversedAt) ?? false,
  });
  ...
  const whatsapp =
    balance > 0
      ? whatsappLink(
          student.parentPhone,
          `${student.name}'s mess fee of ${money(balance)} for ${monthLabel(month)} is pending. Please pay at the mess.`,
        )
      : null;
  ...
  return (
    <div className="flex flex-col gap-5">
      <Link
        href={`/mess/${messId}/fees?month=${monthValue}`}
        className="inline-flex min-h-11 items-center text-base font-semibold text-primary-strong"
      >
        ← Collections
      </Link>
      <header>
      ...
      {whatsapp && (
        <a
          ...
          target="_blank"
          rel="noreferrer"
          className="flex min-h-12 items-center justify-center rounded-xl bg-[#25d366] px-5 text-base font-semibold text-[#05391a]"
        >
              ...
              <li key={entry.id} className="rounded-2xl border-2 border-border bg-white p-4">
                <div className="flex items-start justify-between gap-3">
                  <div>
                    <p className="font-semibold text-text-main">
                      {entryLabel(entry.kind)} · {money(entry.amount)}
                    </p>
                  ...
                  {entry.receiptNumber && !entry.reversedAt && (
                    <Link
                      href={`/mess/${messId}/receipts/${entry.id}`}
                      className="flex min-h-10 items-center rounded-lg bg-muted px-3 text-sm font-semibold text-text-main"
                    >
                      Receipt
                    </Link>
                  )}
                ...
                {entry.reversedAt ? (
                  ...
                  <details className="mt-3">
                    ...
                    <form action={reverseFeeEntry} className="mt-3 flex flex-col gap-2">
                      ...
                      <label className="text-sm font-semibold text-text-main">
                        ...
                        <input
                          ...
                          required
                          minLength={3}
                          className="mt-1 min-h-11 w-full rounded-xl border-2 border-border px-3 text-base font-normal"
                        />
                      </label>
                      <button className="min-h-11 self-start rounded-xl border-2 border-red-800 px-4 text-sm font-semibold text-red-800">
                        Confirm reversal
                      </button>
                    </form>
                  </details>
      ...
      {statement?.reminders.length ? (
        <section>
          <h2 className="font-heading text-xl font-bold text-text-main">Reminder history</h2>
          <ul className="mt-3 flex flex-col gap-2">
            {statement.reminders.map((reminder) => (
              <li key={reminder.id} className="rounded-xl bg-white p-4 text-sm text-text-main">
                <span className="font-semibold">{reminder.status}</span> ·{" "}
                {reminder.createdAt.toLocaleString("en-IN")}
...
function Stat({ label, value }: { label: string; value: string }) {
  return (
    <div className="rounded-2xl border-2 border-border bg-white p-3">
      <p className="break-words font-heading text-lg font-bold text-text-main">{value}</p>
