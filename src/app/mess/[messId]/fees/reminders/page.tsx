...
export default async function ReminderPage({
  ...
  const monthValue =
    raw && /^\d{4}-\d{2}$/.test(raw) ? raw : startOfIstMonth(new Date()).toISOString().slice(0, 7);
  ...
  const students = records
    .map((student) => {
      ...
      const balance = feeBalance(statement?.amount ?? student.monthlyFee, statement?.entries ?? []);
      return {
        id: student.id,
        name: student.name,
        amount: Math.max(0, balance),
        canSend: balance > 0 && Boolean(student.parentPhone) && student.monthlyFee !== null,
      };
    })
    .filter((student) => student.amount > 0);
  return (
    <div>
      <Link
      ...
      <h1 className="mt-2 font-heading text-3xl font-bold text-text-main">Send reminders</h1>
      <p className="mt-1 text-base text-text-muted">
        Choose up to 100 students. Every attempt is recorded, including gateway failures.
      </p>
      <div className="mt-5">
        <BulkReminderForm messId={messId} month={monthValue} students={students} />
      </div>
    </div>
  );
}
