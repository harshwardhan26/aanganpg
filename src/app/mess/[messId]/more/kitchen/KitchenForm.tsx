...
export function KitchenForm({ messId, defaultDay }: { messId: string; defaultDay: string }) {
  ...
  return (
    <form action={action} className="rounded-2xl border-2 border-border bg-white p-5">
      ...
      <h2 className="font-heading text-xl font-bold text-text-main">Record actual food</h2>
      <p className="mt-1 text-sm text-text-muted">
        Save what was prepared and left over after service.
      </p>
      {state.message && (
      ...
      <div className="mt-4 grid gap-4 sm:grid-cols-2">
        <Field label="Date" name="day" type="date" defaultValue={defaultDay} />
        ...
        <Field label="Prepared meals" name="preparedCount" inputMode="numeric" />
        <Field label="Leftover meals" name="leftoverCount" inputMode="numeric" defaultValue="0" />
      </div>
      ...
      >
        {pending ? "Saving…" : "Save kitchen numbers"}
      </button>
    </form>
  );
}
