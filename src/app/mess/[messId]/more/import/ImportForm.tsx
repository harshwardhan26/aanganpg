    ...
    <form action={action} className="mt-5 rounded-2xl border-2 border-border bg-white p-5">
      ...
      {!state.ok && (
        <div role="alert" className="mb-4 rounded-xl bg-red-50 p-4 text-sm text-red-900">
          <p className="font-semibold">Fix these rows:</p>
      ...
      <label className="flex flex-col gap-2 text-sm font-semibold text-text-main">
        Student CSV
        <input
          type="file"
          name="file"
          accept=".csv,text/csv"
          required
          className="min-h-12 rounded-xl border-2 border-dashed border-border p-3 text-base font-normal file:mr-3 file:rounded-lg file:border-0 file:bg-muted file:px-3 file:py-2 file:font-semibold"
        />
      </label>
      <button
        disabled={pending}
