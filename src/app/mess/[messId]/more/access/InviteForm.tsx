...
export function InviteForm({ messId }: { messId: string }) {
  ...
  return (
    <form action={action} className="rounded-2xl border-2 border-border bg-white p-5">
      ...
      <h2 className="font-heading text-xl font-bold text-text-main">Invite someone</h2>
      {state.message && (
        <div role="status" className="mt-3 rounded-xl bg-green-100 p-4 text-sm text-green-900">
          <p>{state.message}</p>
          {state.inviteUrl && <p className="mt-2 break-all font-semibold">{state.inviteUrl}</p>}
        </div>
      )}
      {!state.ok && (
        <p role="alert" className="mt-3 rounded-xl bg-red-50 p-4 text-sm text-red-900">
          {state.issues.join(" ")}
        </p>
      )}
      ...
      <button
        disabled={pending}
        className="mt-5 min-h-12 rounded-xl bg-primary-strong px-5 text-base font-semibold text-white disabled:opacity-60"
      >
        {pending ? "Creating invite…" : "Add or create invite"}
      </button>
    </form>
  );
}
