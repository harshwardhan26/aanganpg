...
export default async function StudentReceiptPage({ params }: { params: Promise<{ messId: string; entryId: string }> }) {
  ...
  return <main className="min-h-screen bg-light px-4 py-6 print:bg-white"><div className="mx-auto mb-5 max-w-xl print:hidden"><Link href={`/my-mess/${messId}/payment`} className="inline-flex min-h-11 items-center font-semibold text-primary-strong">← My fees</Link></div><FeeReceipt receipt={{ receiptNumber: entry.receiptNumber, messName: entry.mess.name, messAddress: entry.mess.address, contactPhone: entry.mess.contactPhone, studentName: entry.payment.student.name, monthLabel: monthLabel(entry.payment.month), amount: entry.amount, method: entry.method, reference: entry.externalReference, receivedAt: entry.occurredAt, reversedAt: entry.reversedAt }} /><p className="mx-auto mt-4 max-w-xl text-center text-sm text-text-muted print:hidden">Use your browser’s Print command to save a PDF.</p></main>;
}
