...
export default async function MessAccessPage({ params }: { params: Promise<{ messId: string }> }) {
    ...
    <div className="mx-auto max-w-md">
      ...
      >
        ...
        <p className="mt-1 text-sm text-text-muted">
          {mess.trialEndsAt
            ? `Trial ends ${mess.trialEndsAt.toLocaleDateString("en-IN")}.`
        ...
        <div className="mt-4 grid grid-cols-2 gap-3">
          <label className="text-sm font-semibold text-text-main">
            ...
            <select
              name="plan"
              defaultValue={mess.subscriptionPlan}
              className="mt-1 min-h-11 w-full rounded-xl border-2 border-border bg-white px-3 text-base font-normal"
            >
          ...
          <label className="text-sm font-semibold text-text-main">
            ...
            <select
              name="status"
              defaultValue={mess.subscriptionStatus}
              className="mt-1 min-h-11 w-full rounded-xl border-2 border-border bg-white px-3 text-base font-normal"
            >
              <option value="TRIAL">Trial</option>
        ...
        <button className="mt-4 min-h-11 rounded-xl bg-primary-strong px-4 text-sm font-semibold text-white">
          Update subscription
        </button>
      </form>
    </div>
